<template>
   ...
   <base-select
    ...
    />
    <div v-if="medical_back.do_you_have_any_diagn_cond==='Yes'" class="tw-mt-2 tw-w-1/2  ">
    ...
    />
     <div v-if="medical_back.do_you_take_any_vitamins==='Yes'" class="tw-mt-2 tw-w-1/2  ">
    <p class="text-h6 tw-ml-10"> If Yes</p>
      <base-input
         class="tw-ml-10 tw-m-2"
        label="Which ones"
       ...
       <div class="tw-flex tw-justify-center tw-mt-4">
         ...
         <q-dialog v-model="open" persistent color="red">
          ...
          >
            <q-card-section >
              <div class="text-h6">Terminer le  process pour Ce patient</div>
            </q-card-section>
            <q-card-section class="q-pt-none">
            </q-card-section>
            <q-card-actions align="right">
              <q-btn label="Valider" color="blue" @click="end_process" />
...
methods:{
    ...
    if( await this.v$.medical_back.which_vaccination.$validate() &&await this.v$.medical_back.is_physical_activity.$validate()){
        return true
    }
    return false
    },
    ...
    async testBackrournd4(){
          if(moment().diff(this.store.currentPatient.date_of_birth, 'years') < 18){
            ...
            if( await this.v$.medical_back.con_tabacco.$validate() &&await this.v$.medical_back.con_alcohol.$validate()){
                return true
            }
          }
    return false
